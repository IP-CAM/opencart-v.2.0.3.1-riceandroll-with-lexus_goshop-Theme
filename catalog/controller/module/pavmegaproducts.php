...
class ControllerModulepavmegaproducts extends Controller {
	...
	public function index($setting) {
		...
		//$mproducts = array();
		//$mega_data = $this->model_pavmegaproducts_product->getProducts( $data );

		//echo "<pre>"; print_r($mega_data); die;

	 	//$mproducts = $this->getProducts( $mega_data, $setting );
		...
		foreach( $setting['category_tabs'] as  $key => $categoryID ){
			...
			$data = array(
	 		...
	 		$mproducts_test[$categoryID] = $this->model_pavmegaproducts_product->getProducts( $data );
		}
		$mproducts = array();
		foreach ($mproducts_test as $category) {
			foreach ($category as $product) {
				$mproducts[] = $product;
			}
		}

		$this->data['megaproducts'] = $this->getProducts( $mproducts, $setting);
